...
단일 파일을 4개 섹션으로 나눠 구성했다.

| 섹션 | 위치(대략) | 책임 |
| --- | --- | --- |
| **1. 프리셋 정의** | L28–166 | `COMMON_ITEMS`, `CATEGORY_TEMPLATES` 등 상수. 여기만 고치면 [프리셋 불러오기]에 반영 |
| **2. 데이터 영속성 + 마이그레이션** | L169–296 | `load_data` / `save` / `ensure_task_fields` — JSON 로드·저장·복구, v1→v2 변환 |
| **3. 업무 다이얼로그** | L303–409 | `TaskDialog` — 제목·유형·카테고리·개수 입력 모달 |
| **4. 메인 애플리케이션** | L416–1089 | `ChecklistApp` — UI 골격, 렌더링, 드래그 정렬, 진행률, 이벤트 처리 |
...
`checklist_data.json` (현재 `version: 2`):
...
- `minsize(320,360)` / `maxsize(560, 화면높이)`로 너비는 위젯답게 제한, 높이는 화면까지 허용.
...
- **프리셋 추가/수정**: `COMMON_ITEMS`, `CATEGORY_TEMPLATES`만 고치면 메뉴에 즉시 반영.
- **카테고리 추가**: `CATEGORY_ORDER`에 추가하고 `CATEGORY_TEMPLATES`에 `초기개발`/`유지보수` 목록 정의.
- **상태/유형 추가**: `STATUS_ORDER`/`KIND_ORDER`와 대응 색상 딕셔너리(`STATUS_COLORS`/`KIND_COLORS`) 갱신.
- **데이터 구조 변경 시**: `DATA_VERSION`을 올리고 `ensure_task_fields`에 변환 분기를 추가.
